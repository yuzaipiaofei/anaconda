...
class BackendTest(mock.TestCase):
    ...
    def anaconda_backend_mount_install_image_1_test(self):
        import pyanaconda.backend
        pyanaconda.backend.shutil = mock.Mock()
        pyanaconda.backend.isys =  mock.Mock()
        pyanaconda.backend.os.path.exists.return_value = True
        pyanaconda.backend.os.path.ismount.return_value = False
        
        FREE_PATH = 'freepath'
        anaconda = mock.Mock()
        anaconda.storage.fsFreeSpace = [[FREE_PATH]]
        installimg = mock.Mock()
        
        ab = pyanaconda.backend.AnacondaBackend(anaconda)
        ab._loopbackFile = True
        ret = ab.mountInstallImage(anaconda, installimg)
        self.assertEqual(None, ret)
        self.assertFalse(anaconda.intf.waitWindow.called)
    
    def anaconda_backend_mount_install_image_2_test(self):
        import pyanaconda.backend
        pyanaconda.backend.shutil = mock.Mock()
        pyanaconda.backend.isys =  mock.Mock()
        pyanaconda.backend.os.path.exists.return_value = False
        pyanaconda.backend.os.path.ismount.return_value = False
        ...
        ab = pyanaconda.backend.AnacondaBackend(anaconda)
        ab._loopbackFile = False
        ret = ab.mountInstallImage(anaconda, installimg)
        self.assertEqual(None, ret)
        self.assertTrue(anaconda.intf.waitWindow.called)
        self.assertTrue(pyanaconda.backend.isys.lochangefd.called)
        self.assertFalse(pyanaconda.backend.isys.umount.called)
    
    def anaconda_backend_mount_install_image_3_test(self): 
        import pyanaconda.backend
        pyanaconda.backend.shutil = mock.Mock()
        pyanaconda.backend.isys =  mock.Mock()
        pyanaconda.backend.os.path.exists.return_value = False
        pyanaconda.backend.os.path.ismount.return_value = True
        ...
        self.assertTrue(anaconda.intf.waitWindow.called)
        self.assertTrue(pyanaconda.backend.isys.lochangefd.called)
        self.assertTrue(pyanaconda.backend.isys.umount.called)
        
    def anaconda_backend_mount_install_image_4_test(self): 
        import pyanaconda.backend
        pyanaconda.backend.shutil = mock.Mock()
        pyanaconda.backend.shutil.copyfile.side_effect = Exception
        pyanaconda.backend.os.path.exists.return_value = False
        ...
        self.assertTrue(pyanaconda.backend.os.unlink.called)
    
    def anaconda_backend_remove_install_image_test(self):
        import pyanaconda.backend
        anaconda = mock.Mock()
        ab = pyanaconda.backend.AnacondaBackend(anaconda)
        ab._loopbackFile = True
        ab.removeInstallImage()
        self.assertTrue(pyanaconda.backend.os.unlink.called)
    
    def anaconda_backend_freetmp_1_test(self):
        import pyanaconda.backend
        pyanaconda.backend.os.path.exists.return_value = False
        anaconda = mock.Mock()
        ab = pyanaconda.backend.AnacondaBackend(anaconda)
        ret = ab.freetmp(anaconda)
        self.assertEqual(None, ret)
    
    def anaconda_backend_freetmp_2_test(self):
        import pyanaconda.backend
        pyanaconda.backend.DISPATCH_BACK = -1
        pyanaconda.backend.os.path.exists.return_value = True
        pyanaconda.backend.AnacondaBackend.mountInstallImage = mock.Mock()
        
        anaconda = mock.Mock()
        ab = pyanaconda.backend.AnacondaBackend(anaconda)
        ret = ab.freetmp(anaconda)
        self.assertEqual(pyanaconda.backend.DISPATCH_BACK, ret)
    
    def anaconda_backend_freetmp_3_test(self):
        import pyanaconda.backend
        pyanaconda.backend.os.path.exists.return_value = True
        pyanaconda.backend.AnacondaBackend.mountInstallImage = mock.Mock(
            return_value=False)
    ...
    def anaconda_backend_freetmp_4_test(self):
        import pyanaconda.backend
        pyanaconda.backend.DISPATCH_BACK = -1
        pyanaconda.backend.os.path.exists.return_value = True
        pyanaconda.backend.AnacondaBackend.mountInstallImage = mock.Mock(
            return_value=False)
        pyanaconda.backend.os.unlink.side_effect = SystemError
